package cc.cloudcoding.auth.domain.service.gitee.impl;
...
import cc.cloudcoding.auth.domain.service.gitee.GiteeUUIDUserDetailService;
 ...
 */
@Service
public class GiteeUUIDUserDetailServiceImpl implements GiteeUUIDUserDetailService {
    ...
    public UserDetails loadUserByUuid(String uuid) throws UsernameNotFoundException {
        AuthorizationPO authorizationPO = authorizationMapper.selectOne(new LambdaQueryWrapper<AuthorizationPO>().eq(AuthorizationPO::getIdentifier, uuid).eq(AuthorizationPO::getIdentityType, LoginType.GITEE.getValue()));
        if (Objects.isNull(authorizationPO)) {
            throw new UsernameNotFoundException("用户未绑定本平台用户！");
        }
