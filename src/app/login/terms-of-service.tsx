...
const component: React.FC<{ visible: boolean, onChange: (b: boolean) => void }> = ({ visible, onChange }) => {
    const footerContent = (
        <div>
            <Button label="我已知晓" icon="pi pi-check" onClick={() => onChange(false)} autoFocus />
        </div>
    );
    return (

        <Dialog header="服务协议与隐私权政策" visible={visible} style={{ width: '50vw' }} onHide={() => onChange(false)} footer={footerContent}>
            <p className="mb-5">
