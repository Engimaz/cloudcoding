...
import type { FeatureVO, Organization, OrganizationRecord, OrganizationRecordAggregate, OrganizationVO, Url, UrlVO } from './types.ts'
...
export const getOrganizationList = (page: number, size: number, keyword: string, status?: string) => {
    let url;
    if (status) {
    ...
    const method = "get";
    return request<QueryListResult<OrganizationVO>>(url, method, { page, size, keyword });
}
// 新增一个组织
...
export const getFeatureList = (page: number, size: number, keyword: string) => {
    ...
    const method = "get";
    return request<QueryListResult<FeatureVO>>(url, method, { page, size, keyword });
}
...
// 新增功能
export const insertFeature = (data: FeatureVO) => {
...
// 更新功能
export const updateFeature = (data: FeatureVO) => {
